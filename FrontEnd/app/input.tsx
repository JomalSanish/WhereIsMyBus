...
export default function HomeScreen() {
  ...
  return (
    <View style={darkTheme.container}>
      <View>
        <Text style={darkTheme.textbusdetails}>Enter the 'From' and 'To' locations:</Text>
        <TextInput
      ...
      <View>
          ...
          <View style={{alignItems: "center", width: "100%", height: 70, zIndex: 10, position: "absolute"}}>
            {
              fromsuggestions.map((fromsugg) => {
                return(
                  <TouchableOpacity onPress={() => setFrom(fromsugg.name)} key={fromsugg._id} style={{backgroundColor: "#121212", padding:10, borderColor: "#148f57", borderWidth: 1, width: "100%", borderRadius: 6}}>
                  <Text style={{color: "white"}}>{fromsugg.name}</Text>
                  </TouchableOpacity>
                )
              })
            }
          ...
          <View style={{alignItems: "center", width: "100%", height: 70, zIndex: 10, position: "absolute", top: -21}}>
            {
              tosuggestions.map((tosugg) => {
                return(
                  <TouchableOpacity onPress={() => setTo(tosugg.name)} key={tosugg._id} style={{backgroundColor: "#121212", padding:10, borderColor: "#148f57", borderWidth: 1, width: "100%", borderRadius: 6}}>
                  <Text style={{color: "white"}}>{tosugg.name}</Text>
                  </TouchableOpacity>
                )
              })
            }
        ...
        <Button title="Search" color="#148f57" onPress={handleSearch} />
      </View>
      </View>
  );
}
